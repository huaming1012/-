...
void mul(int** A, int** B, int** C, int n, int m, int p){
    for(int i = 0; i < n; i++){
        for(int j = 0; j < p; j++){
            C[i][j] = 0;
            for(int k = 0; k < m; k++){
...
int main(){
    ...
    clock_t startTime_Mul;
    clock_t endTime_Mul;

    clock_t startTime_Strassen;
    clock_t endTime_Strassen;
    srand(time(0));

    cout << "朴素矩阵算法开始时钟：" << (startTime_Mul = clock()) << endl;
    mul(A, B, C, n, m ,p);
    cout << "朴素矩阵算法结束时钟：" << (endTime_Mul = clock()) << endl;
 /*   for(int i = 0; i < n; i++){
        for(int j = 0; j < p; j++){
            printf("%d ", C[i][j]);
        }
    ...
    cout << "Strassen矩阵算法开始时钟：" << (startTime_Strassen = clock()) << endl;
    Strassen(N, A, B, C);
    cout << "Strassen矩阵算法结束时钟：" << (endTime_Strassen = clock()) << endl;
/* 
    for(int i = 0; i < n; i++){
        for(int j = 0; j < p; j++){
            printf("%d ", C[i][j]);
    ...
    cout << "朴素矩阵算法：" << (endTime_Mul - startTime_Mul) << " Clocks.." << (endTime_Mul - startTime_Mul)/CLOCKS_PER_SEC << " Sec" << endl;
    cout << "Strassen矩阵算法：" << (endTime_Strassen - startTime_Strassen) << " Clocks.." << (endTime_Strassen - startTime_Strassen)/CLOCKS_PER_SEC << " Sec" << endl;
